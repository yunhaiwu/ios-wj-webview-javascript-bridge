...
@property WJWebViewJavascriptBridge* bridge;
@end
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    _bridge = [WJWebViewJavascriptBridge bridgeForWebView:webView];
    [_bridge setWebViewDelegate:self];
    
    [_bridge registerHandler:@"testObjcCallback" handler:^(id data, WJWVJBResponseCallback responseCallback) {
        NSLog(@"testObjcCallback called: %@", data);
        responseCallback(@{@"t1":@"v1",@"t2":@"v2",@"t3":@"v3",@"t4":@"v4",@"t5":@"v5"});
    }];
